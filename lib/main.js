...
function keyCodeFromEvent(event) {
  let {KeyEvent} = event.target.ownerDocument.defaultView;
...
function findOverlay(event) {
  ...
  for (let id of Object.keys(gOverlays)) {
    if (keycode == gOverlays[id].keycode && sameModifiers(modifiers, gOverlays[id].modifiers)) {
      return id;
    }
  }
...
function handlePossibleShortcut(event) {
  ...
  if (overlay) {
    ...
    // TODO overlay=true for overriden shortcuts
  }
