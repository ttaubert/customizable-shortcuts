...
let {modifierKeys, Modifiers} = require("modifiers");
...
Shortcut.prototype.equals = function (obj) {
  ...
  if (obj.stopPropagation) {
    if (this.modifiers && !this.modifiers.equals(obj))
      return false;

    if (this.key)
      return (this.key == obj.keyCode);

    return (Ci.nsIDOMKeyEvent["DOM_" + this.keycode] == obj.keyCode);
  }

  return false;
}

Shortcut.prototype.serialize = function () {
  return {key: this.key, keycode: this.keycode, modifiers: this.modifiers && this.modifiers.serialize()};
}
...
Shortcut.unserialize = function (data) {
  return new Shortcut({
    key: data.key,
    keycode: data.keycode,
    modifiers: data.modifiers && Modifiers.unserialize(data.modifiers)
  });
}
