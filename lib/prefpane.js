...
let ShortcutsPane = function (window) {
  let createElement = function (tag, parent) {
  ...
  let hbox = createElement("hbox", this.element);
  ...
  let sep = createElement("separator", this.element);
  sep.setAttribute("class", "thin");

  this.element.appendChild(tree);
  window.setTimeout(function () tree.view = new TreeView(), 0);
}
...
PreferenceTreeRenderer.prototype.render = function () {
  ...
  this.window.addEventListener("keydown", function (event) {
    ...
    let index = tree.view.getCellValue(editingRow, editingColumn);
