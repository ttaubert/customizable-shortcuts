...
PreferenceTreeRenderer.prototype.render = function () {
  ...
  tree.addEventListener("keydown", function (event) {
    ...
    let {editingColumn, editingRow} = tree;

    tree.stopEditing(true);
    tree.view.setCellText(editingRow, editingColumn, shortcut.toString());

    if (shortcut.isComplete()) {
      new Overlay({key: Keys.keys[editingRow], shortcut: shortcut});
    } else {
      tree.startEditing(editingRow, editingColumn);
    }
  }, true);
...
PreferenceTreeRenderer.prototype.renderKey = function (key) {
  ...
  cell2.setAttribute("editable", true);
