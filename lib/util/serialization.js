...
let serialize = function (data) {
  ...
  if (data instanceof require("core/overlay").Overlay)
...
let unserialize = function (data) {
  ...
  if ("overlay" == data._type)
    return new require("core/overlay").Overlay({key: unserialize(data.key), shortcut: unserialize(data.shortcut)}, {dontStore: true});
