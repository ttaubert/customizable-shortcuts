...
let TreeView = function (filter) {
  ...
  for (let [name, group] in Iterator(Groups.groupKeys(this.keys))) {
    let container = {type: "group", name: name, parent: -1};
    let parent = this.rows.push(container) - 1;

    group.forEach(function (key) {
      self.rows.push({type: "key", key: key, parent: parent})
    });
  }
}
...
TreeView.prototype = {
  ...
  getCellValue: function (idx, column) {
    if (!column.index || this.isContainer(idx))
      return;

    return Keys.keys.indexOf(this.rows[idx].key);
  },
  ...
  isContainerOpen: function (idx) true,
